...
(function(w, $) {
	...
	function bindCardData($card, data) {
		...
		tags.forEach(function(t) {
			var cls = '';
			if (t.indexOf('未') !== -1) cls = 'warn'; // 未出貨或未付款 → 標示警告色
	...
	function createCard(data) {
		...
		var detailHtml = '';var kv = [];

		// 一律列出（空值顯示 —），若要跳過空值可把下一行改成：if (v == null || v === '') return;
		DETAIL_FIELDS.forEach(function(fieldName) {
			var v = (data && Object.prototype.hasOwnProperty.call(data, fieldName)) ? data[fieldName] : '';
			if (v == null || v === '') v = '—'; //
			kv.push('<div class="kv"><b>' + fieldName + '</b><span>' + String(v) + '</span></div>');
		});
