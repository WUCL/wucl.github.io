    ...
    var FORM = {
        ...
        bindEvent: function() {
            ...
            $this.el.$pics.$btn_pic_upload.on('change', function(e) {
                ...
                if ($this.var.imgFile.status != '') return alert('圖片處理中');

                let $current = event.currentTarget;
                let $whichone = $current.getAttribute('id');
                let $preview_el = 'preview_' + $whichone;
                let $tempid = $('#' + $preview_el).attr('data-tempid') || $this.var.imgFile.exif.counter;
                console.log($tempid);
                // let $b64 = '';
                ...
                if (file && file.name) {
                    EXIF.getData(file, function() {
                        ...
                        if (exifData) {
                            ...
                            let counter = $tempid;
                            $this.var.imgFile.exif[counter] = '';
                            $this.var.imgFile.exif[counter] = {'file_create_time': '','lat': '','lng': ''};
                            $this.var.imgFile.exif[counter]['file_create_time'] = file_create_time;
                            $this.var.imgFile.exif[counter]['lat'] = lat;
                            $this.var.imgFile.exif[counter]['lng'] = lng;
                        } else {
                ...
                async function handleImageUpload(file) {
                    $this.var.imgFile.status = 'uploading';
                ...
                // step3, do api call
                function doImgSync() {
                    $this.var.imgFile.status = '';
                    $this.var.imgFile.exif.counter += 1;
